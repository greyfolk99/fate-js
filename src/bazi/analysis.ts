...
const GROUP_OF: Record<TenGod, StarGroup> = {
  "비견": "비겁", "겁재": "비겁",
  "편인": "인성", "정인": "인성",
  "식신": "식상", "상관": "식상",
  "편재": "재성", "정재": "재성",
...
/** 일간 오행 기준 각 오행군의 오행. */
function groupElements(dm: Element): Record<StarGroup, Element> {
  // 인성 = 일간을 생하는 오행, 관성 = 일간을 극하는 오행.
  let generator: Element = dm
  for (const e of ELEMENTS) if (GENERATES[e] === dm) generator = e
  return {
    비겁: dm,
    인성: generator,
    식상: GENERATES[dm],
...
export interface StrengthFacts {
  ...
  deukryeong: { present: boolean; monthBranchTenGod: TenGod }
  /** 득지(得地) — 일지에 일간의 뿌리(비겁·인성 지장간)가 있는가. */
  deukji: { present: boolean; roots: { stem: Stem; tenGod: TenGod }[] }
  ...
  reference: { verdict: "신강" | "신약" | "중화"; basis: string }
}

export interface GyeokgukFacts {
  monthBranch: Branch
  monthHiddenStems: { stem: Stem; tenGod: TenGod; role: string }[]
  /** 월지 지장간 중 천간에 투출한 것. */
  revealed: { stem: Stem; tenGod: TenGod; role: string; atStems: PillarName[] }[]
  /** 격 후보(단정 아님) — 투출 우선, 없으면 월령 본기. */
  candidates: { name: string; tenGod: TenGod; basis: string }[]
}
...
function roleAt(count: number, idx: number): string {
...
function strengthFacts(bazi: Bazi): StrengthFacts {
  ...
  // 득지 — 일지 지장간에 비겁·인성.
  const dayRoots: { stem: Stem; tenGod: TenGod }[] = []
  for (const hs of HIDDEN_STEMS[bazi.day.branch]) {
    const g = tenGod(dm, hs)
    if (GROUP_OF[g] === "비겁" || GROUP_OF[g] === "인성") dayRoots.push({ stem: hs, tenGod: g })
  }
  const deukji = { present: dayRoots.length > 0, roots: dayRoots }

  // 득세 — 세력 카운트(일간 자신 제외).
  const tally = (withHidden: boolean) => {
    const byGroup: Record<StarGroup, number> = { 비겁: 0, 인성: 0, 식상: 0, 재성: 0, 관성: 0 }
    for (const c of cs) {
      if (c.name !== "day") byGroup[GROUP_OF[tenGod(dm, c.stem)]]++
      if (withHidden) {
        for (const hs of HIDDEN_STEMS[c.branch]) byGroup[GROUP_OF[tenGod(dm, hs)]]++
      } else {
  ...
  // 투출 — 지장간이 천간에 드러남.
  const stemPositions = new Map<Stem, PillarName[]>()
  for (const c of cs) {
    if (!stemPositions.has(c.stem)) stemPositions.set(c.stem, [])
    stemPositions.get(c.stem)!.push(c.name)
  }
  const revealed: StrengthFacts["revealed"] = []
  for (const c of cs) {
    for (const hs of HIDDEN_STEMS[c.branch]) {
      const at = stemPositions.get(hs)
      if (at) revealed.push({ stem: hs, fromBranch: c.name, atStems: at })
    }
  }
...
function gyeokName(g: TenGod): string {
...
function gyeokgukFacts(bazi: Bazi): GyeokgukFacts {
  ...
  const candidates: GyeokgukFacts["candidates"] = []
  if (revealed.length) {
...
const SEASON_OF: Record<Branch, string> = {
  "寅": "봄(春)", "卯": "봄(春)", "辰": "봄(春)",
