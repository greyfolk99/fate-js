...
function birthUtcSec(input: BirthInput): number {
  ...
  const offsetMin = input.utcOffsetMinutes !== undefined
    ...
    : resolveUtcOffsetMinutes(
        input.timezone as string, year, month, day, stdHour, stdMinute,
      )
...
function searchSortedRight(arr: number[], val: number): number {
  ...
  while (lo < hi) {
    ...
    if (arrMid === undefined || arrMid <= val) lo = mid + 1
    else hi = mid
  }
  return lo
}
...
export function daeun(
  input: BirthInput & { gender: Gender },
  count = 10,
): Daeun {
 ...
 */
export function seun(startYear: number, count = 5): SeunYear[] {
  const out: SeunYear[] = []
