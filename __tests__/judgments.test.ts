...
import { matchSheet, tenGod, nayinOf } from "../src/match/index.js"
import { analyze } from "../src/index.js"
import type { Bazi, Pillar, Stem, Branch } from "../src/index.js"
import type { MatchSubject } from "../src/match/index.js"

// ── 조립기 (match.test.ts 와 동일 패턴) ────────────────────────────
...
describe("십성 분포·교차", () => {
  it("분포는 일간 자신을 빼고 나머지 천간을 집계", () => {
    ...
    const t = matchSheet({ bazi: a }, { bazi: a })
  ...
  it("교차 읽기: 후보 일간이 주체에게 무슨 십성인가", () => {
    ...
    const t = matchSheet({ bazi: subj }, { bazi: cand })
...
describe("배우자성(재/관) 공급", () => {
  it("남명 주체의 배우자성=재성을 후보가 공급하면 present", () => {
    ...
    const subj: MatchSubject = {
      bazi: bz(["戊", "戌"], ["戊", "戌"], ["甲", "子"]),
      gender: "male",
    }
    const cand: MatchSubject = { bazi: bz(["己", "丑"], ["戊", "戌"], ["乙", "亥"]) }
    const t = matchSheet(subj, cand)
    expect(t.judgments.tenGod.spouseStarForSubject.present).toBe(true)
  })

  it("gender 미제공이면 배우자성 판정 보류(present:false)", () => {
    const subj: MatchSubject = { bazi: bz(["戊", "戌"], ["戊", "戌"], ["甲", "子"]) }
    const cand: MatchSubject = { bazi: bz(["己", "丑"], ["戊", "戌"], ["己", "丑"]) }
    const t = matchSheet(subj, cand)
  ...
  it("배우자성이 후보 지지의 지장간에만 있어도 공급으로 잡힌다", () => {
    ...
    const cand: MatchSubject = { bazi: bz(["甲", "寅"], ["甲", "寅"], ["甲", "寅"]) }
    const t = matchSheet(subj, cand)
    expect(t.judgments.tenGod.spouseStarForSubject.present).toBe(true)
  })
})
...
describe("judgments 스키마", () => {
  it("matchSheet 에 judgments 섹션이 붙고 12관계와 분리된다", () => {
    const t = matchSheet(
      { bazi: bz(["甲", "子"], ["甲", "子"], ["甲", "子"]) },
      { bazi: bz(["己", "丑"], ["己", "丑"], ["己", "丑"]) },
    )
