 ...
 */
import { STEM_ELEMENTS, BRANCH_ELEMENTS, STEM_YINYANG, STEM_INDEX, BRANCH_INDEX, BRANCHES, HIDDEN_STEMS, STEM_COMBINATIONS, BRANCH_LIUHE, BRANCH_CLASH, BRANCH_HARM, } from "../constants.js";
import { STEM_CLASH, BRANCH_PA, BRANCH_WONJIN, BRANCH_SAMHAP, BRANCH_BANGHAP, BRANCH_HYUNG, } from "../match/constants.js";
import { CHEONEUL_BY_STEM, MUNCHANG_BY_STEM, HONGYEOM_BY_STEM, YANGIN_BY_STEM, BAEKHO_PILLARS, GWAEGANG_PILLARS, GWIMUN_PAIRS, } from "../match/judgments-constants.js";
...
function stemSinsal(id, label, targets, bazi, basis) {
    const pillars = [];
...
function pillarSinsal(id, label, set, bazi) {
    const pillars = [];
    for (const c of cells(bazi)) {
        if (set.has(c.stem + c.branch))
            pillars.push(c.name);
    }
    return { id, label, present: pillars.length > 0, pillars, basis: "간지(주) 자체" };
}
/** 원국 내 귀문관살 — 두 지지가 귀문 쌍을 이루는가. */
function gwimunSinsal(bazi) {
    const cs = cells(bazi);
    const pillars = new Set();
    for (let i = 0; i < cs.length; i++) {
        for (let j = i + 1; j < cs.length; j++) {
            const paired = GWIMUN_PAIRS.some((s) => s.has(cs[i].branch) && s.has(cs[j].branch));
            if (paired) {
...
const HYUNG_PAIRS = (() => {
    ...
    for (const rule of BRANCH_HYUNG) {
        ...
        if (rule.kind === "jahyung" && b[0])
            m.set(b[0] + b[0], rule.name);
    }
    return m;
})();
function internalRelations(bazi) {
    const cs = cells(bazi);
    const out = [];
    // 두 기둥 쌍(pairwise) 관계.
    const pairSpecs = [
...
// ── 오행 분포 ───────────────────────────────────────────────────────
function elementDistribution(bazi) {
