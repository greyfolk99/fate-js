import { cells, pairwiseRules, hyungRule, samhapRule, banghapRule, hiddenAmhapRule, elementComplementRule, } from "./rules.js";
 ...
 */
export function matchSheet(subject, candidate) {
    ...
    const lenses = LENS_ORDER.map((lens) => {
...
function hanjaOf(label) {
    const m = label.match(/\(([^)]+)\)/);
    return m ? m[1] : label;
}
/** 영문 오행 배열 → 한자 문자열. */
function elsHanja(list) {
    if (!Array.isArray(list))
        return "";
    return list.map((e) => ELEMENT_HANJA[e] ?? String(e)).join("");
}
const PILLAR_HANJA = {
    year: "年", month: "月", day: "日", hour: "時",
};
/** 엣지 → "A日子-B日丑" — 어느 사주·어느 기둥의 글자끼리 성립했는지(궁위 사실). */
function edgeHanja(e) {
 ...
 * 형태의 사실이지 강도 점수가 아니다.
 */
function groupSegments(name, edges) {
    const byEl = new Map();
    for (const e of edges) {
        const el = ELEMENT_HANJA[e.element] ?? String(e.element);
 ...
 */
export function formatMatchSheet(sheet, opts) {
    const lines = [];
    for (const g of sheet.lenses) {
        ...
        const parts = active.flatMap((f) => {
            ...
            const SAMHYUNG_GROUPS = {
                無恩之刑: ["寅", "巳", "申"],
                恃勢之刑: ["丑", "戌", "未"],
            };
            const kinds = f.id === "branch_hyung" && Array.isArray(f.detail?.hyung)
                ? `(${f.detail.hyung.map((k) => {
                    const group = SAMHYUNG_GROUPS[k];
                    if (!group)
                        return k;
                    const glyphs = new Set(f.edges.flatMap((e) => [e.subject.glyph, e.object.glyph]).filter((g) => group.includes(g)));
        ...
        if (g.lens === "生") {
            ...
            if (opts?.includeHarm) {
                const harm = (jd, tag, keukTag) => {
                    const d = jd.detail;
                    parts.push(d.covered.length
    ...
    if (opts?.includeWangswe && j.chungWangswe.present) {
        const entries = (j.chungWangswe.detail?.entries ?? []);
    ...
    if (j.palace.present) {
        ...
        aux.push(`궁위 겉(和${d.outerHarmony ?? 0}沖${d.outerClash ?? 0})` +
    ...
    if (s2.length)
        aux.push(`神殺(B→A):${s2.join("·")}`);
    if (s1.length)
        aux.push(`神殺(A→B):${s1.join("·")}`);
    if (aux.length)
        lines.push(`【보조】 ${aux.join(" · ")}`);
    ...
    const dayStemRel = eA === eB ? `比和(${dsA}·${dsB})`
    ...
    if (cr.dayPillarMatch.present)
        cx.push(`일주:${cr.dayPillarMatch.category}`);
    ...
    if (ov.present) {
        const cells = ov.detail?.cells ?? [];
        lines.push(`【병존】 ${cells.join(" · ")}`);
    }
    // 쟁합·투합 — 한 글자가 같은 종류 합을 여럿 맺어 힘이 갈라지는 사실.
    const jh = j.jaenghap;
    if (jh.present) {
        const cells = jh.detail?.cells ?? [];
        lines.push(`【쟁합】 ${cells.join(" · ")}`);
    }
    return lines.join("\n");
}
//# sourceMappingURL=matchsheet.js.map